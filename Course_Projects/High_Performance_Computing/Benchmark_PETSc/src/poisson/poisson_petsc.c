...
PetscErrorCode ComputeRHS(KSP ksp, Vec b, void *ctx) {
  ...
  for (int i = info.ys; i < info.ys + info.ym; i++) {
    for (int j = info.xs; j < info.xs + info.xm; j++) {
      array[i][j] = - user->c;
    }
  }
...
PetscErrorCode ComputeMatrix(KSP ksp, Mat A, Mat P, void *ctx) {
  ...
  // > LOOP OVER GRID (i)
    // > LOOP OVER GRID (j)
      ...
      // // Bottom: S_{i,j-1}
      // PetscScalar v_bottom = 1 / hy2;
      
      // // Left: S_{i-1,j}
      // PetscScalar v_left   = 1 /hx2;
      
      // // Center: S_{i,j}
      // PetscScalar v_center = -2/ hy2 - 2/hx2 ;
      
      // // Right: S_{i+1,j}
      // PetscScalar v_right = 1/hx2;
      
      // // Top: S_{i,j+1}
      // PetscScalar v_top = 1 /hy2;
...
int main(int argc, char **argv){
  ...
  Vec         b, x;
  PetscMPIInt rank;
  PetscInt    nx = 8, ny = 4; // Default grid size .. 
  PetscBool   flg;
