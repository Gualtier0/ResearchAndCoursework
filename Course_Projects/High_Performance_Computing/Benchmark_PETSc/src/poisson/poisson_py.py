 ...
 *     python poisson_py.py -nx 64 -ny 64 -solver sp_dir,dn_dir,sp_cg
...
def ComputeRHS(nx, ny, f_value):
    ...
    rhs = np.full((nx, ny), f_value)  #rn this just give him 20
    rhs[0, :] = 0 
    rhs[-1, :] = 0
    rhs[:, 0] = 0
    rhs[:, -1] = 0
...
def ComputeMatrix(nx, ny, dx, dy):
    ...
    for row in range(ny):
        for col in range(nx):
            index = row * nx + col  # Flattened index for the current grid point
