...
void write_binary(std::string fname, Field &u, SubDomain &domain,
  ...
  int start[2]    = {domain.starty - 1, domain.startx - 1};
  ...
  MPI_File_set_view(filehandle, disp, MPI_DOUBLE, filetype, "native",
                    MPI_INFO_NULL);
  MPI_File_write_all(filehandle, u.data(), domain.ny*domain.nx, MPI_DOUBLE,
                     MPI_STATUS_IGNORE);
...
int main(int argc, char* argv[]) {
 ...
 int size = 1, rank = 0, len;
    ...
    // Display the new welcome message only from rank 0
    if (rank == 0) {
        printf("Welcome to mini-stencil!\n");
        printf("version   :: C++ MPI\n");
        printf("processes :: %d\n", size);
    }
