...
double hpc_dot(Field const& x, Field const& y) {
    ...
    //i gather and sum result locals and broadcast to result
    for (int i = 0; i < N; i++) {
        result_local += x[i] * y[i];
    }
...
double hpc_norm2(Field const& x) {
    ...
    int N = x.length();
    // compute local result
    //indagate if u can do also result= hpc_dot(x, x) after this probably good idea idk
    for (int i = 0; i < N; i++) {
        result_local += x[i] * x[i];
