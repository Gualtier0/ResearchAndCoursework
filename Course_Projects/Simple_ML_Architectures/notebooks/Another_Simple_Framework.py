...
class ReLU(Layer):
    ...
    def backward(self, grad_in: np.ndarray) -> Layer.BackwardResult:
        ...
        d_x = grad_in * (relu_x > 0) ##der relu is 1 for positive or 0
...
class Linear(Layer):
    ...
    def backward(self, grad_in: np.ndarray) -> Layer.BackwardResult:
        ...
        dW = x.T @ grad_in
        db = grad_in.sum(axis=0) #need to sum the in batch size
...
class SoftmaxCrossEntropy(Loss):
    def forward(self, inputs: np.ndarray, targets: np.ndarray) -> float:
        ...
        crossentropy = -np.sum(one_hot_t * np.log(softmax), axis=1)  #should i add something small here to smax to avoid log (0)???
        mean_crossentropy = np.mean(crossentropy)
    ...
    def backward(self) -> np.ndarray:
        ...
        ## Implement
        softmaxhelp = softmax - one_hot_t
        d_inputs =  softmaxhelp / len(softmax)
        ## End
...
def create_network() -> Layer:
    ...
    network = Sequential([
        Linear(2, 50),  # in.layer 2 in 50 out
        ReLU(),
        Linear(50, 30),  # hid layer 50 in 30 out
        ReLU(),
        Linear(30, 2)   # out layer 30 in 2 out
      ])
...
def verify(images: np.ndarray, targets: np.ndarray) -> Tuple[int, int]:
    ...
    num_ok = np.sum(np.argmax(model.forward(images), axis=1) == targets)
...
best_epoch = -1
batch_n = (len(train_validation_set) + batch_size - 1) // batch_size
...
for epoch in range(200):
    ...
    for i in range(0, len(train_indices), batch_size):
      accumulated_loss = train_one_step(model, loss, learning_rate,
                                        train_validation_set.images[train_indices[i: i + batch_size]],
                                        train_validation_set.labels[train_indices[i: i + batch_size]])

    current_loss_value /= 1 + len(train_indices) // batch_n
    current_loss_value += accumulated_loss
